# Repo-demo
Day-1 of learning git and github
<br> 
br- is for new line
<br>
Clone => cloning a repo on our local machine, syntax => git clone link enter - to clone this int vs code, it creates a folder
<br>
cd, change directory(folder),to move from git to the folder 
<br>
ls, list files, to see/check all the files/folders, it lists all the files and folders
<br>
to see hidden files => "ls -a" 
<br>
status => display the status of the code, syntax => git status.
<br>
4 types of statuses, Untracked, Modified, Staged, Unmodified.<br>
Untracked=new files git doesn't yet track, Modified=changed, Staged=file ready to be committed<br>
Add => adds new or changed files in your working directory to the git staging area<br> 
Add syntax => git add filename 
Commit => is is the record of change, SYNTAX => git commit -m "some message"


